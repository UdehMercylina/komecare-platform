<?php
    ...
    if(isset($_POST['join'])){
        ...
        if($row['numrows'] > 0){
            ...
            try{
                $stmt = $conn->prepare("INSERT INTO users (full_name, firstname, lastname, email, phone_no, gender, address, town_city, postcode, activate_code, created_on) VALUES (:full_name, :firstname, :lastname, :email, :phone_number, :gender, :address, :town_city, :postcode, :code, :now)");
                $stmt->execute(['full_name'=>$full_name, 'firstname'=>$firstname, 'lastname'=>$lastname, 'email'=>$email, 'phone_number'=>$phone_number, 'gender'=>$gender, 'address'=>$address, 'town_city'=>$town_city, 'postcode'=>$postcode, 'code'=>$code, 'now'=>$now]);
                ...
                $msg = $lastname." ".$firstname." from ".$town_city." just registered, Log into the admin center to review and take necessary actions";
                ...
                try {
                    ...
                    $mail->Subject = $settings->siteTitle." Onboarding";  
                    $mail->Body = $message;
